...
## 配置代理
### Quick Start
最简单的方法：`https://github.com/nelvko/clash-for-linux-install` 一键安装，一键导入，还提供webui进行订阅管理，香飞了。一键安装链接：
...
其中，`gh-proxy.org/`为加速前缀，在有 github 加速的场景，例如 autodl 上，可以关闭。通过设置:
...
### clashctl的原理
其原理是现在服务器上拉取完整的clash-for-linux-install项目，再执行项目中的安装脚本`install.sh`，在home dictionary下新建clashctl文件夹完成代理相关的所有配置。安装完成后，clash-for-linux-install文件夹就可以删除啦。

项目默认的配置文件在`/env'和`.env.install`中，建议在安装时增加限时，防止下载超时。
## ssh端口转发
